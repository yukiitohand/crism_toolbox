...
if isempty(range_latd)
    ...
    range_latd = [...
        min(DEdata.ddr.Latitude.img(valid_lines,valid_samples),[],'all') - mrgn_deflt*10, ...
        max(DEdata.ddr.Latitude.img(valid_lines,valid_samples),[],'all') + mrgn_deflt*10];
end

if isempty(range_lond)
    latmean = mean(DEdata.ddr.Latitude.img(valid_lines,valid_samples),'all');

 ...
 * 12 lList_exist
 * 13 lList_crange
 * 14 dyu                  int8_t flag for dynamic linked list update.
 *
 * 
 * 
 * 
        ...
        int32_t *lList_exist, int32_t **lList_crange)
{
    ...
    l0 = lList_exist[0]; lend = lList_exist[1]+1;
    ...
    for(l=l0;l<lend;l++){
        ...
        s0 = lList_crange[l][0]; send = lList_crange[l][1]+1;
        for(c=s0;c<send;c++){
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    int8_T **msldemc_imFOVmask;
    double S_im,L_im;
    int32_t *lList_exist, **lList_crange;
    int8_t dyu;
    ...
    */
    ...
    lList_exist = mxGetInt32s(prhs[12]);
    lList_crange = set_mxInt32Matrix(prhs[13]);
    
    /* INPUT 12 Dynamic Linked List Update FLAG */
    dyu = (int8_t) mxGetScalar(prhs[14]);
    ...
    bin_msldemt_ctr_iaumars_L2PBK_LL0_4CRISM(S_im, L_im, cahv_mdl,
            ...
            K_L,K_S,
            ll_papmc_bin, &ll_napmc,lList_exist,lList_crange);
    end_time = clock();
    ...
    if(dyu==1){
        mask_obstructed_pts_in_msldemt_using_msldemc_iaumars_L2PBK_LL0DYU_M3_4CRISM(
            ...
            S_im, L_im, cahv_mdl,lList_exist,lList_crange);
    } else if(dyu==0){

/* =====================================================================
 * crism_gale_get_msldemFOV_direct_mex.c
 ...
 * 
 * OUTPUTS:
 * 0 msldemc_imFOVmask    int16 [L_dem x S_dem]
 *    5: inside the FOV (srange[0]<x<srange[1] && lrange[0]<y<lrange[1])
 *    4: tightest margin of the FOV (any of 8 neighbors is marked as 5)
 *    0: outside FOV
 *   -1: invalid dem value
 *
 * 1 crism_FOVcell        cell array [Ncrism]
 * 2 crismPxl_srngs       valid samples [2 x Ncrism]
 * 3 crismPxl_lrngs       valid lines   [2 x Ncrism]
 * 
 *
 *
...
void get_imFOVmask_MSLDEM_direct(float **msldem_img_radius, double mslradius_offset,
        ...
        int16_t **msldem_imFOVmask, mxArray *crism_FOVcell, int16_t Ncrism,
        int32_t **crismPxl_srngs, int32_t **crismPxl_lrngs)
{
    ...
    for(l=0;l<msldemc_lines;l++){
        ...
        for(c=0;c<msldemc_samples;c++){
            ...
            // radius    = (double) msldem_img_radius[(sample_offset+c)*msldem_lines+l+line_offset] + mslradius_offset;
            if(isnan(radius)){
                msldem_imFOVmask[c][l] = -2;
            } else {
                ...
                if(apmc>0){
                    ...
                    if(y_im>y_min && y_im<y_max){
                        ...
                        if(x_im>x_min && x_im<x_max){
                            xi=0;
                            while( x_im>PmCbrd_imxap[xi] ){
                                xi++;
                            }
                            xi--;
                            msldem_imFOVmask[c][l] = xi;
                            if(c<crismPxl_srngs[xi][0]) {
                                crismPxl_srngs[xi][0] = c;
                            } else if(c>crismPxl_srngs[xi][1]) {
                                crismPxl_srngs[xi][1] = c;
                            }
                            if(l<crismPxl_lrngs[xi][0]) {
                                crismPxl_lrngs[xi][0] = l;
                            } else if(l>crismPxl_lrngs[xi][1]) {
                                crismPxl_lrngs[xi][1] = l;
                            }
                        }
                    }
                } else {
    ...
    for(xi=0;xi<Ncrism;xi++){
        soffset = crismPxl_srngs[xi][0]; loffset = crismPxl_lrngs[xi][0];
        sz_c = crismPxl_srngs[xi][1] - soffset+1;
        sz_l = crismPxl_lrngs[xi][1] - loffset+1;
        ...
        for(c=0;c<sz_c;c++){
            for(l=0;l<sz_l;l++){
                if(msldem_imFOVmask[c+soffset][l+loffset]==xi){
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    */
    ...
    msldem_imFOVmask = set_mxInt16Matrix(plhs[0]);
    ...
    get_imFOVmask_MSLDEM_direct(msldem_img_radius, mslradius_offset,
            ...
            msldem_imFOVmask,crism_FOVcell, (int16_t) Ncrism,
            crismPxl_srngs,crismPxl_lrngs);

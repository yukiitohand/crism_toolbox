/* =====================================================================
 * crism_gale_get_msldemFOVcell_PFF_L2_mex.c
...
//             if(msldemc_imFOVmask[c][l]>4){
                 ...
                 * pixel */
                xii=0; dy2 = y_im*y_im;
                /* Evaluate crismPxl_srngs and crismPxl_lrngs */
                while( xii<Ncrism && x_im>PmCbrd_imxap_min[xii]){
                    if(x_im<PmCbrd_imxap_max[xii]){
                        //if(c<crismPxl_srngs[xii][0] || c>crismPxl_srngs[xii][1] 
                        //        || l<crismPxl_lrngs[xii][0] || l>crismPxl_lrngs[xii][1]){
                  ...
                  int nrhs, const mxArray *prhs[])
{
     ...
     * ----------------------------------------------------------------- */
    if(nrhs!=14) {
        mexErrMsgIdAndTxt("crism_gale_get_msldemFOVcell_PFF_L2_mex:nrhs","14 inputs required.");
    }
    if(nlhs!=3) {
        mexErrMsgIdAndTxt("crism_gale_get_msldemFOVcell_PFF_L2_mex:nlhs","3 outputs required.");
    }
    
    /* make sure the first input argument is scalar */
    
    if( !mxIsChar(prhs[0]) ) {
        mexErrMsgIdAndTxt("crism_gale_get_msldemFOVcell_PFF_L2_mex:notChar","Input 0 needs to be a character vector.");
    }

function [search_result] = crism_search_products(obs_id, product_type, varargin)
...
if (rem(length(varargin),2)==1)
    ...
    for i_sgid=1:2:(length(varargin)-1)
        switch upper(varargin{i_sgid})
            case 'OBS_CLASS_TYPE'
                obs_class_type = varargin{i_sgid+1};
            case 'OBS_COUNTER'
                obs_counter = varargin{i_sgid+1};
            case 'ACTIVITY_ID'
                activity_id = varargin{i_sgid+1};
            case 'ACTIVITY_MACRO_NUM'
                activity_macro_num = varargin{i_sgid+1};
            case 'SENSOR_ID'
                sensor_id = varargin{i_sgid+1};
            % case 'YYYY_DOY'
            %     yyyy_doy = varargin{i+1};
            case 'VERSION'
                vr = varargin{i_sgid+1};
            case {'EXT','EXTENSION'}
                ext = varargin{i_sgid+1};
            case {'DWLD','DOWNLOAD'}
                dwld = varargin{i_sgid+1};
            case 'OVERWRITE'
                overwrite = varargin{i_sgid+1};
            case 'INDEX_CACHE_UPDATE'
                index_cache_update = varargin{i_sgid+1};
            otherwise
                error('Unrecognized option: %s', varargin{i_sgid});   
        end
    end
end
...
[dir_info,basenames,fnamewext_local] = crism_search_observation_fromProp(...
    ...
    'overwrite',overwrite,'CellOutput',true);
